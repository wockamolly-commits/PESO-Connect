...
**Goal:** Add a full settings page with account management (password reset, delete account), notification preferences, and privacy controls.

**Architecture:** Single `Settings.jsx` page with tabbed sections. `deleteAccount` added to AuthContext. Notification and privacy settings stored as Firestore objects on the user doc. Auto-save with debounce for toggles.

**Tech Stack:** React 18, Firebase Auth, Firestore, Tailwind CSS, Lucide React

---

### Task 1: Add `deleteAccount` to AuthContext
...
Update the Firebase Auth import (lines 2-7) to include `reauthenticateWithCredential`, `EmailAuthProvider`, and `deleteUser`:
...
Also add `deleteDoc` to the Firestore import (line 8):

```jsx
import { doc, setDoc, getDoc, onSnapshot, updateDoc, deleteDoc } from 'firebase/firestore'
```

**Step 2: Add `deleteAccount` function**

After the `resetPassword` function, add:
...
**Step 3: Add `deleteAccount` to context value**

In the `value` object, add `deleteAccount` after `resetPassword`:
...
git commit -m "feat: add deleteAccount with re-authentication to AuthContext"
```

---

### Task 2: Create Settings.jsx page

**Files:**
- Create: `src/pages/Settings.jsx`

**Step 1: Create the full Settings component**

Create `src/pages/Settings.jsx` with:
...
git commit -m "feat: add settings page with account, notifications, and privacy tabs"
```

---

### Task 3: Add route and Navbar link

**Files:**
- Modify: `src/App.jsx`
- Modify: `src/components/Navbar.jsx`

**Step 1: Add Settings import and route to App.jsx**

Add import after the Messages import:
```jsx
import Settings from './pages/Settings'
```

Add a protected route after the Messages routes:
```jsx
                        <Route
                            path="/settings"
                            element={
...
In `src/components/Navbar.jsx`, add a Settings link in the desktop navigation user section. Find the Logout button (around line 155) and add a Settings link before it:
...
Note: The `Settings` icon is already imported from lucide-react on line 8.

**Step 3: Commit**

```bash
git add src/App.jsx src/components/Navbar.jsx
git commit -m "feat: add /settings route and settings link to navbar"
```

---

### Task 4: Respect privacy settings in PublicProfile.jsx

**Files:**
- Modify: `src/pages/PublicProfile.jsx`

**Step 1: Add privacy gate for profile visibility**

In `PublicProfile.jsx`, after fetching the profile data, add a check: if `profile.privacy_settings?.profile_visibility === 'verified_only'` and the viewing user is not verified, show a restricted access message instead of the profile.

After the loading check and before the main profile render, add:

```jsx
    // Privacy: check profile visibility settings
    const isRestricted = profile?.privacy_settings?.profile_visibility === 'verified_only'
        && (!userData?.is_verified)
```

If `isRestricted`, show a card saying "This profile is only visible to verified users."

**Step 2: Wrap skills section with privacy check**

Find the skills display (around line 130) and wrap it:
```jsx
...
git commit -m "feat: respect privacy settings in public profile"
```
